...
export async function runFisioTriage({ patientName, specialty, anamnesis, media }) {
  const config = getConfig();
  const system = buildSystemPrompt(config);

  let contextText = `## DADOS DO PACIENTE\n`;
  contextText += `Nome: ${patientName || '(não informado)'}\n`;
  if (specialty) contextText += `Especialidade/Área: ${specialty}\n`;
  if (anamnesis && anamnesis.trim()) {
    contextText += `\n### Anamnese / Textos do grupo\n${anamnesis}\n`;
  }
  contextText += `\nAnalise todos os exames enviados abaixo. Siga rigorosamente o protocolo de avaliação fisioterapêutica.`;
